...
在多代码做了多次剥离后，最终剩下了上面这一部分代码人，看了一下，没发现问题。
...
<img src="%E4%B8%80%E6%AC%A1glibc%E7%9A%84%E5%86%85%E5%AD%98%E9%9B%AA%E5%B4%A9%E6%83%A8%E6%A1%88%E8%AE%B0.assets/image-20200628191106715.png" alt="image-20200628191106715" style="zoom: 67%;" />
...
在明白了这些之后我们在看遇到的问题： 在发生问题的python代码中，我们每次循环调用都创建了一个新的`DataFrame` 。可以确定的是：在`pandas`的底层调用`glibc` 为数据分配内存时使用了一个很小的`chunk`但是在释放时应为`glibc` 的问题没有正常释放，导致这个极小的内存碎片没法正确释放给操作系统，积少成多，造成严重的内存泄漏。
